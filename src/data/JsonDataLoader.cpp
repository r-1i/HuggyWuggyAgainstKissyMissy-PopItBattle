...
std::vector<HeroTemplate> JsonDataLoader::loadHeroes(int tavernLevel) {
  ...
  for (const auto& item : j) {
    if (item.value("minTavernLevel", 1) > tavernLevel) continue;
...
std::vector<RoomTemplate> JsonDataLoader::loadRooms() {
  ...
  for (const auto& item : j) {
    ...
    r.possibleEnemyTypes =
        item.at("possibleEnemyTypes").get<std::vector<std::string>>();
    result.push_back(std::move(r));
  }
  return result;
}

LootTable JsonDataLoader::loadLootTable(const std::string& tableName) {
